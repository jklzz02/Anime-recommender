...
def recommend(anime_id: int, limit: int = Body(default=10, ge=1, le=100)):
    ...
    if not results:
        raise HTTPException(
            status_code=404, detail="Anime not found or no similar entries."
        )
...
def recommend_batch_detailed(
    anime_ids: list[int], limit: int = Body(default=10, ge=1, le=100)
):
...
@router.get(
    "/recommend/semantic/search",
    response_model=list[RecommendationResponse],
    tags=["NLP Search"],
)
def recommend_from_text_semantic(
    query: str = Query(
        ..., description="Natural language query like 'action anime with magic'"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing text query: {e!s}"
        )


@router.get(
    "/recommend/semantic/search/detailed",
    response_model=list[RecommendedAnime],
    tags=["NLP Search"],
)
...
@router.get(
    "/recommend/text", response_model=list[RecommendationResponse], tags=["NLP Search"]
)
