...
_anime_data_instance: Optional[AnimeDataLoader] = None

def get_anime_data_loader() -> AnimeDataLoader:
    """Get or create the global AnimeDataLoader instance"""
    global _anime_data_instance
    if _anime_data_instance is None:
        _anime_data_instance = AnimeDataLoader()
        try:
            _anime_data_instance.load()
            logger.info(f"✓ Loaded {_anime_data_instance.anime_count} anime entries")
        except AnimeDataLoaderError as e:
            logger.error(f"✗ Failed to load anime data: {e}")
    return _anime_data_instance
