...
def _validate_and_get_indices(user_anime_ids: list[int]) -> tuple[list[int], list[int]]:
    ...
    content_user_indices = [
        id_to_index[str(aid)] for aid in user_anime_ids if str(aid) in id_to_index
    ]
...
def _compute_weighted_average(
    ...
    if user_ratings:
        weights = np.array(
            [
                max(
                    0, user_ratings.get(id_getter(aid) if id_getter else aid, 7.0) - 5.0
                )
                / 5.0
...
def _compute_similarity_with_weights(
...
def _compute_final_similarity(
    cf_similarity: float | None, content_similarity: float | None, cf_boost: float = 1.2
) -> float | None:
    """
...
def calculate_compatibility_score(
    ...
    user_ratings: dict[int, float] | None = None,
    ...
    if target_cf_idx is not None and cf_user_ids:
        ...
        user_cf_vecs = anime_cf_embeddings[
            [anime_to_cf_idx[aid] for aid in cf_user_ids]
        ]
    ...
    if content_user_indices:
        target_content_vec = compatibility_embeddings[int(target_content_idx)].reshape(
            1, -1
        )
    ...
    final_similarity = _compute_final_similarity(
        cf_similarity, content_similarity, cf_boost
    )
...
def get_most_compatible_from_favourites(
    user_anime_ids: list[int],
    limit: int = 10,
    user_ratings: dict[int, float] | None = None,
    score_floor: float = 50.0,
...
def get_cf_recommendations_from_favorites(
    user_anime_ids: list[int],
    limit: int = 10,
    user_ratings: dict[int, float] | None = None,
) -> list[tuple[int, float]]:
    ...
    pseudo_user_vector = _compute_weighted_average(
        user_cf_vecs, cf_user_ids, user_ratings
    )
...
def get_hybrid_recommendations_from_favorites(
    user_anime_ids: list[int],
    limit: int = 10,
    user_ratings: dict[int, float] | None = None,
    cf_weight: float = 0.5,
    ...
    cf_recs = get_cf_recommendations_from_favorites(
        user_anime_ids, limit=limit * 3, user_ratings=user_ratings
    )
    ...
    if content_user_indices:
        ...
        similarities = cosine_similarity(
            user_profile, compatibility_embeddings
        ).flatten()
...
def get_hybrid_recommendations_with_text_from_favorites(
    user_anime_ids: list[int],
    text_query: str,
    limit: int = 10,
    user_ratings: dict[int, float] | None = None,
    ...
    query_embedding = model.encode(
        [query_with_prefix], show_progress_bar=False, normalize_embeddings=True
    )[0].reshape(1, -1)

    nlp_similarities = cosine_similarity(query_embedding, nlp_embeddings).flatten()
    nlp_scores = {
        int(index_to_id[i]): float(nlp_similarities[i])
        for i in range(len(nlp_similarities))
    }

    all_ids = (
        set(cf_scores.keys()) | set(content_scores.keys()) | set(nlp_scores.keys())
    )
    hybrid_results = []
    for anime_id in all_ids:
        cf_score = cf_scores.get(anime_id, 0.0)
...
def predict_rating_from_favorites(
    user_anime_ids: list[int],
    anime_id: int,
    user_ratings: dict[int, float] | None = None,
) -> float | None:
...
def get_similar_users_from_favorites(
    user_anime_ids: list[int],
    limit: int = 10,
    user_ratings: dict[int, float] | None = None,
) -> list[tuple[int, float]]:
...
def hybrid_text_search(
    query: str, limit: int = 10, nlp_weight: float = 0.6, content_weight: float = 0.4
) -> list[tuple[int, float]]:
    """
    ...
    combined_scores = (
        nlp_weight * nlp_similarities + content_weight * content_similarities
    )
